...
node *AVL::insert(node *root, string str_w, string str_m)
{
    if (root == NULL)
    {
        node *temp = new node;
    ...
    else
    {
        ...
        {
            root->right = insert(root->right, str_w, str_m);
            if (BF(root) == -2)
                root = (str_w.compare(root->right->word) > 0) ? RR(root) : RL(root);
        }
        else
        {
            root->left = insert(root->left, str_w, str_m);
            if (BF(root) == 2)
                root = (str_w.compare(root->left->word) < 0) ? LL(root) : LR(root);
        }
    }
    root->ht = height(root);
    return root;
...
int main()
{
    ...
    while (true)
    {
        ...
        case 3:
            ...
            cout << "\n Ascending Order:"
                 << "\n";
            Tree.displayAscending(Tree.root);
            cout << "\n Descending Order:"
                 << "\n";
            Tree.displayDescending(Tree.root);
            break;
        case 4:
            cout << "Enter word: ";
            cin >> str1;
